"""
완전 로컬 판 데모 앱  -  외부 API 를 하나도 부르지 않는다.

    app_gpu_tabs.py       1·2·6 단계를 Gemini 로, 4 단계를 크로스인코더로.
    app_gpu_tabs_Qwen.py  네 단계 전부 로컬 모델로. API 키가 필요 없다.

무엇이 GPU 에 올라가나 (전부 프로세스당 한 번, lru_cache):

    1·2 재작성/확장   Qwen/Qwen3-8B            bf16 약 16GB
    3   질의 임베딩    harrier-oss-v1-0.6b      약 1.2GB
    4   리랭킹        bge-reranker-v2-m3       약 1.1GB
    6   답변 생성      Qwen/Qwen3-8B            위와 같은 인스턴스를 재사용
                                               ------------------------
                                               합계 약 18GB

24GB GPU 를 기준으로 잡았다. 그보다 작으면 Qwen3-8B 를 4bit 로 올리거나
Qwen3-4B 로 낮춘다 (LOCAL_LLM_MODEL 환경변수로 바꿀 수 있다).

Gemini 판과 견줘 달라지는 것:

  - API 키와 네트워크가 필요 없다. 폐쇄망에서도 돈다.
  - JSON 스키마 강제가 사라진다. Gemini 는 response_schema 로 형식을
    보장하지만 로컬 모델은 그렇지 않아, src/local_llm.py 가 코드펜스를 벗기고
    괄호 짝을 맞춰 잘라내고 실패하면 온도 0 으로 한 번 더 시도한다.
  - 첫 검색이 느리다. 16GB 모델을 올리는 시간이 붙는다.
  - 답변 품질은 재봐야 한다. 특히 근거가 없을 때 "모른다"고 말하는 능력이
    Gemini 보다 떨어질 수 있다. src/grade.py --run all 로 18문항 정답률을
    Gemini 판과 견주면 바로 드러난다.

    streamlit run app_gpu_tabs_Qwen.py --server.address 0.0.0.0 --server.port 8501
"""
...
# src/rerank_gpu.py 로 넘긴다.
RERANK_METHOD = "cross"

# 1·2·6 단계를 로컬 Qwen3-8B 로 돌린다. "gemini" 로 두면 API 를 부른다.
# 모델을 바꾸려면 LOCAL_LLM_MODEL 환경변수를 쓴다 (예: Qwen/Qwen3-4B-Instruct).
LLM_BACKEND = "qwen"
...
def cached_pipeline(question: str, lang: str,
    ...
    네 단계 모두 로컬 모델이라 외부 호출이 없다. 대신 매번 GPU 추론이 도므로
    캐시가 더 중요하다. Streamlit 은 위젯을 건드릴 때마다 스크립트를 처음부터
    다시 도는데, 캐시가 없으면 화면을 조금만 움직여도 Qwen3-8B 생성이 두 번
    (재작성·답변) 다시 돈다.
    """
    return run_pipeline(question, lang=lang, top_k=top_k, final_n=final_n,
                        rerank_method=RERANK_METHOD, llm_backend=LLM_BACKEND)
...
def render_rerank(rr) -> None:
    """4. 리랭킹 — 등수가 어떻게 바뀌었는지"""
    method = {
        "llm": "Gemini",   # 이 앱에서는 쓰이지 않는다 (cross 고정)
...
NEXT_STEPS = [
    ...
    ("guided decoding 도입",
     "로컬 모델은 JSON 형식이 보장되지 않아 지금은 출력에서 괄호 짝을 맞춰 "
     "잘라내고 실패 시 재시도한다. vLLM·outlines 의 문법 강제 디코딩으로 "
     "옮기면 파싱 실패 자체가 사라짐."),
]
...
with tab_demo:
    ...
    if search_clicked:
        ...
        result = run_pipeline(selected_question, lang=lang_code, gold=gold,
                              ...
                              llm_backend=LLM_BACKEND, on_stage=on_stage)
