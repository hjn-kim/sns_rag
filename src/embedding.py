...
# -*- coding: utf-8 -*-
"""
JSON 문서 임베딩 (Harrier / dense / .npz)

--data 로 지정한 폴더 안의 JSON 을 하나씩 순차로 처리한다.
본문을 토큰 수 기준으로 청킹한 뒤 Harrier 임베딩 모델로 dense 벡터를 만들어
파일마다 .npz 하나를 남긴다.

    data/output_gpu/2024_마약류_범죄백서.json
      -> data/output_gpu/2024_마약류_범죄백서_embeddings.npz
    data/output_gpu/2024_마약류_범죄백서_영어.json
      -> data/output_gpu/2024_마약류_범죄백서_영어_embeddings.npz
    ...

모델은 한 번만 올려 모든 파일에 재사용한다.
이미 .npz 가 있으면 건너뛰므로 중간에 끊겨도 다시 실행하면 이어서 진행한다.

청킹은 모델 토크나이저 기준이다. 문자 수가 아니라 실제로 모델이 보는
토큰 수로 자르므로 --chunk-size 가 모델 입력 길이와 정확히 일치한다.

사용 예:
    pip install -r requirements.txt

    python embedding.py                                  # data/output_gpu 전체 (500/100)
    python embedding.py --data data/output_cpu
    python embedding.py --data data/output_gpu/2024_마약류_범죄백서_영어.json  # 한 파일만
    python embedding.py --chunk-size 300 --overlap 50
    python embedding.py --limit 20                       # 파일당 앞 20청크만 (테스트용)
    python embedding.py --dry-run                        # 모델 없이 청킹 결과만 확인

저장 형식 (.npz):
    embeddings   float32 (N, dim)  L2 정규화된 dense 벡터
    texts        <U      (N,)      청크 원문
...
# 본문으로 볼 key 후보 (--field 를 주지 않았을 때 순서대로 찾는다)
TEXT_KEY_CANDIDATES = ("text", "content", "body")

# 본문을 못 찾아 전체를 훑을 때 건너뛸 key (경로/도구 이름 등)
SKIP_KEYS = {"source", "method", "backend", "document_format", "document_date"}
...
def collect_strings(node, key: str | None = None) -> list[str]:
    """dict/list 를 재귀적으로 훑어 문자열 value 만 모은다."""
    ...
    if isinstance(node, list):
        ...
        return out
    if isinstance(node, str) and key not in SKIP_KEYS and node.strip():
        return [node]
    return []


def extract_text(data, field: str | None) -> tuple[str, str]:
    """
    (본문, 어디서 뽑았는지) 를 돌려준다.

    --field 가 있으면 그 key 를, 없으면 text/content/body 를 순서대로 찾는다.
    그래도 없으면 모든 문자열 value 를 이어붙인다.
    """
    if isinstance(data, dict):
        if field:
            if field not in data:
                sys.exit(f"'{field}' key 가 JSON 에 없습니다. 있는 key: {list(data)}")
            value = data[field]
            if not isinstance(value, str):
                sys.exit(f"'{field}' 가 문자열이 아닙니다: {type(value).__name__}")
            return value, field
    ...
    parts = collect_strings(data)
    if not parts:
        sys.exit("번역/임베딩할 문자열을 찾지 못했습니다.")
    return "\n\n".join(parts), "(전체 문자열)"
...
def chunk_by_tokens(text: str, tokenizer, size: int, overlap: int) -> list[dict]:
    ...
    슬라이딩 간격(stride)은 size - overlap 이다. 예: 500/100 -> 400 씩 전진.
    """
    if overlap >= size:
...
def collect_files(data: Path) -> list[Path]:
    """--data 가 파일이면 그 파일만, 폴더면 안의 *.json 을 이름순으로."""
    if data.is_file():
        return [data]
    return sorted(data.rglob("*.json"))


def build_output_path(src: Path, data_root: Path, out_dir: Path) -> Path:
    """폴더 모드에서는 원본의 하위 폴더 구조를 그대로 유지한다."""
    if data_root.is_file():
        return out_dir / f"{src.stem}_embeddings.npz"
    rel = src.relative_to(data_root)
    return out_dir.joinpath(*rel.parts[:-1]) / f"{src.stem}_embeddings.npz"
...
def run_file(src: Path, dst: Path, tokenizer, model, device: str, args) -> dict | None:
    ...
    with src.open(encoding="utf-8") as fp:
        data = json.load(fp)
    text, origin = extract_text(data, args.field)
    ...
    started = time.time()
    # Harrier 는 검색 대상 문서에 instruction 프리픽스를 붙이지 않는다.
    # (쿼리 쪽에만 prompt_name="web_search_query" 등을 붙인다)
    ...
    # 모델이 bfloat16 으로 돌면 정규화 결과가 노름 1 에서 0.2% 정도 어긋난다.
    # 내적을 그대로 코사인 유사도로 쓰려면 float32 에서 한 번 더 맞춰야 한다.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="JSON 문서를 토큰 기준으로 청킹해 Harrier dense 임베딩을 만든다.",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
        "--data", default="data/output_gpu", type=Path,
        help="임베딩할 JSON 파일 또는 폴더 (기본: data/output_gpu)\n"
             "폴더면 안의 *.json 을 이름순으로 하나씩 처리한다",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"임베딩 모델 (기본: {DEFAULT_MODEL})\nCPU 라면 microsoft/harrier-oss-v1-270m 이 훨씬 빠르다.",
    )
    parser.add_argument(
        "--chunk-size", type=int, default=500,
        help="청크당 토큰 수 (기본: 500)",
    )
    parser.add_argument(
        "--overlap", type=int, default=100,
        help="앞 청크와 겹치는 토큰 수 (기본: 100)",
    )
    parser.add_argument(
        "--batch-size", type=int, default=32,
        help="한 번에 인코딩할 청크 수 (기본: 32)\n"
             "500토큰 x 32 는 24GB(A5000) 에서 여유롭다. CPU 면 8 정도로 낮출 것",
    )
    parser.add_argument(
        "--device", default=None,
    ...
    parser.add_argument(
        "--dtype", default="auto",
        help="모델 연산 dtype: auto / float32 / bfloat16 (기본: auto)\n"
             "auto 는 모델 설정값(bfloat16)을 따른다. A5000 은 Ampere 라 bf16 을\n"
             "네이티브로 돌리므로 auto 가 가장 빠르다.",
    )
    parser.add_argument(
    ...
    out_dir: Path = (args.out or (data_root.parent if data_root.is_file()
                                  else data_root)).resolve()

    files = collect_files(data_root)
    if not files:
        sys.exit(f"JSON 파일이 없습니다: {data_root}")
    ...
    if not args.dry_run:
        print("모델 로드 중... (최초 실행 시 다운로드에 시간이 걸립니다)")
    ...
    for idx, src in enumerate(files, 1):
        dst = build_output_path(src, data_root, out_dir)
