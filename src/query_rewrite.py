...
질문 하나를 넣으면 한 번의 호출로 둘을 받는다.
    ...
    재작성  검색에 맞게 다듬은 질의 하나. 손댈 필요가 없으면 원 질문 그대로.
    확장    다른 각도로 바꾼 질의 3개.
...
# 이 톡방의 고유 용어. 프롬프트에 넣어두면 약어를 제대로 풀어 쓴다.
GLOSSARY = """
- MDA = Ministries, Departments and Agencies (나이지리아 정부 부처/기관)
- PBI = Power BI (마이크로소프트 시각화 도구)
- CP = capital project (자본 사업)
- Resagratia = 이 프로젝트를 운영하는 데이터 분석 커뮤니티 이름
- house = 이 방에서 참여자 전체를 부르는 호칭
"""

SYSTEM_PROMPT = f"""당신은 RAG 검색의 질의 재작성기입니다.

검색 대상은 WhatsApp 그룹 채팅 로그입니다. 나이지리아 연방정부 예산 문서(PDF)에서
부처별 데이터를 추출해 Power BI 로 시각화하는 프로젝트를 함께 하는 사람들의 대화이며,
튜토리얼 안내 · 작업 분담 · 오류 질문과 답변 · 워크숍 공지가 오갑니다.
...
[재작성] 검색에 쓸 질의 하나
  - 질문형을 **서술형으로** 바꾸세요. 색인 대상이 대화라서 질문형으로 검색하면
    답변이 아니라 "같은 질문을 한 메시지"가 걸립니다. 답변처럼 생긴 문장이어야
    답이 담긴 대목과 가까워집니다.
  - 약어와 일반 명사를 이 방에서 실제로 쓰는 표현으로 바꾸세요
    (예: "부처" -> "MDA", "시각화 도구" -> "Power BI").
  - 이미 검색에 적합하면 **원 질문을 그대로 두세요.** 억지로 바꾸지 마세요.

[확장] 서로 다른 각도의 질의 {N_EXPANSIONS}개
  - 1번: 대상 언어({{language}})로 옮긴 질의. 색인이 그 언어이므로 어휘를 맞춥니다.
  - 2번: 핵심 키워드만 남긴 짧은 질의.
  - 3번: 질문자가 알고 싶은 것을 다른 표현·다른 초점으로 바꾼 질의.
  - {N_EXPANSIONS}개가 서로 겹치지 않게 하세요.

원 질문에 없는 사실을 지어내지 마세요. 날짜·이름·수치를 임의로 채우지 마세요."""
...
RESPONSE_SCHEMA = {
    ...
    "properties": {
        "rewritten": {
            "type": "string",
            "description": "검색에 쓸 재작성 질의. 바꿀 필요가 없으면 원 질문 그대로.",
        },
        "changed": {
            "type": "boolean",
